...
assert.match(html, /Software Engineer\s*<span>Building Intelligent, Scalable Systems\.<\/span>/);
assert.match(html, /Over 5 years of experience taking software from idea to production, including federal systems\./);
assert.match(html, /I work across requirements, system design, full-stack implementation/);
assert.match(html, /AI-assisted development/);
assert.match(html, /secure, scalable, production-grade federal systems/);
...
assert.match(html, /"focus"<\/span>: "federal systems · modernization · security"/);
...
assert.match(html, /portfolio\/assets\/img\/ppic\.jpg 1344w/);
assert.match(html, /loading="lazy"/);
assert.match(html, /decoding="async"/);
assert.match(html, /ppic-560\.jpg 560w/);
assert.match(html, /photography\/mobile\/dsc00236\.jpg 600w/);
assert.match(html, /photography\/optimized\/dsc00236\.jpg 900w/);
...
assert.match(sadafCaseStudyHtml, /viewport-fit=cover/);
assert.match(sadafCaseStudyHtml, /class="nav-toggle"/);
...
assert.match(css, /\/\* Mobile experience \*\//);
assert.match(css, /\.mobile-nav-ready \.site-header:not\(\.nav-open\) \.main-nav/);
...
assert.doesNotMatch(sadafCaseStudyHtml, /fonts\.(?:googleapis|gstatic)\.com/);
assert.doesNotMatch(heroPrototypeHtml, /fonts\.(?:googleapis|gstatic)\.com/);
...
assert.match(mainJs, /setupMatrixIntro\(\{ onReveal: setupEnhancements \}\)/);
assert.match(matrixIntroJs, /requestAnimationFrame/);
assert.match(matrixIntroJs, /prefers-reduced-motion: reduce/);
assert.match(matrixIntroJs, /window\.innerWidth < 600/);
assert.match(matrixIntroJs, /matchMedia\("\(max-width: 680px\)"\)/);
...
assert.match(mainJs, /import\("\.\/carousel\.js"\)/);
assert.match(mainJs, /IntersectionObserver/);
assert.match(readProjectFile("portfolio/assets/js/carousel.js"), /mobileSource/);
assert.match(matrixIntroJs, /portfolio-intro-seen/);
assert.match(heroJs, /prefers-reduced-motion: reduce/);
assert.match(heroJs, /max-width: 680px/);
...
assert.match(uiJs, /setupMobileNavigation\(root\)/);
assert.match(uiJs, /classList\.toggle\("nav-open"/);
...
assert.match(heroPrototypeHtml, /src="portfolio\/assets\/img\/ppic-672\.jpg"/);
assert.match(heroPrototypeHtml, /portfolio\/assets\/img\/ppic\.jpg 1344w/);
